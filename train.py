...
from model_single_frame import SimpleBaseline
from common.eval_util import mpjpe, compute_errors
...
for epoch in range(num_epochs):
    ...
    for i, (data, label) in enumerate(train_dataloader):
        ...
        loss = mpjpe(output.view(-1, 23, 3), label.view(-1, 23, 3))
    ...
    with torch.no_grad():
        ...
        for i, (data, label) in enumerate(val_dataloader):
            ...
            pa_body_loss = compute_errors(label.detach().cpu().numpy().reshape(-1, 23, 3) * 1000.0,
                                          output.detach().cpu().numpy().reshape(-1, 23, 3) * 1000.0)
        ...
        if curr_val_loss < best_eval_loss:    # 只要损失下降就保存
            ...
            torch.save({
                ...
                },root_path + 'checkpoint/model_{}_loss_{:.6f}.pt'.format(epoch, curr_val_loss))
